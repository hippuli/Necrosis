...
function OVERLAY.SPELL_AURA(self, ...)
...
function OVERLAY.COMBAT_LOG_EVENT_UNFILTERED(self, ...)
...
function OVERLAY.LOADING_SCREEN_DISABLED(self, ...)
...
function OVERLAY.SPELL_UPDATE_USABLE(self, ...)
    self:CheckAllCounterActions();
end

-- Specific spellbook update
function OVERLAY.SPELLS_CHANGED(self, ...)
...
function OVERLAY.LEARNED_SPELL_IN_TAB(self, ...)
    local spellID, skillInfoIndex, isGuildPerkSpell = ...;
    self:LearnNewSpell(spellID);
end

-- Event receiver
function OVERLAY.OnEvent(self, event, ...)
    if self[event] then
        self[event](self, ...);
    end
