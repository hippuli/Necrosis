...
-- Register a new aura
-- If texture is nil, no Spell Activation Overlay (OVERLAY) is triggered; subsequent params are ignored until glowIDs
-- If texture is a function, it will be evaluated at runtime when the OVERLAY is triggered
-- If glowIDs is nil or empty, no Glowing Action Button (GAB) is triggered
-- All OVERLAY arguments (between spellID and b, included) mimic Retail's SPELL_ACTIVATION_OVERLAY_SHOW event arguments
