...
function OVERLAY.LoadDB(self)
    ...
    if not db.classes then
        ...
        db.classes = CopyTable(OVERLAY.defaults.classes);
    else
        -- Subsequent initializations will deep-merge from defaults
        for classFile, classData in pairs(OVERLAY.defaults.classes) do
    ...
    if (self.CurrentClass) then
        self.CurrentClass.Register(OVERLAY);
    end
end
...
loader:SetScript("OnEvent", function (event)
    OVERLAY:LoadDB();
    OVERLAY:ApplyAllVariables();
    SpellActivationOverlayOptionsPanel_Init(OVERLAY.OptionsPanel);
